import subprocess
from mutagen.mp4 import MP4, MP4FreeForm, MP4Tags  # Importación actualizada
import os
...
def update_metadata(file_path):
    ...
    # Registrar claves freeform personalizadas
    MP4Tags.register_freeform_key(
        name="com.apple.quicktime.ChromaSubsampling",
        key="----:com.apple.quicktime:ChromaSubsampling"
    )
    MP4Tags.register_freeform_key(
        name="com.apple.quicktime.BitsPerComponent",
        key="----:com.apple.quicktime:BitsPerComponent"
    )
    ...
    # Actualizar metadatos técnicos con formato correcto
    video.tags["\xa9too"] = [MP4FreeForm("HEVC h.265 4:2:2 10-bit".encode("utf-8"))]
    video.tags["----:com.apple.quicktime:ChromaSubsampling"] = [
        MP4FreeForm("4:2:2".encode("utf-8"))
    ]
    video.tags["----:com.apple.quicktime:BitsPerComponent"] = [
        MP4FreeForm("10".encode("utf-8"))
    ]
    
    # Preservar fecha original
    if "\xa9day" in video.tags:
        original_date = video.tags["\xa9day"]
        video.tags["\xa9day"] = original_date
    
    # Mantener perfil de color existente
    video.tags["----:com.apple.quicktime:ColorProfile"] = [
        MP4FreeForm(bytes.fromhex('6E636C7801010101010100'))
    ]
